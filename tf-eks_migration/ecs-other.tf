resource "aws_ecs_cluster" "content-other" {
  name = "production-other"
}

resource "aws_launch_configuration" "content-other" {
  name          = "content-other"
...
resource "aws_autoscaling_group" "content-other" {
  name                      = "content-other"
  min_size                  = var.public.ecs-other.min_size
  max_size                  = var.public.ecs-other.max_size
  health_check_type         = "EC2"
  launch_configuration      = aws_launch_configuration.content-other.name
  vpc_zone_identifier       = [
  ...
  tags = [
    {
      key                 = "Name"
      value               = "ecs-content-other"
      propagate_at_launch = "1"
    }
  ]
}

resource "aws_autoscaling_policy" "content-other-add-instance" {
  name                   = "content-other-add-instance"
  scaling_adjustment     = 1
  adjustment_type        = "ChangeInCapacity"
  cooldown               = 300
  autoscaling_group_name = aws_autoscaling_group.content-other.name
}

resource "aws_cloudwatch_metric_alarm" "content-other-memory-warning" {
  alarm_name = "content-other-memory-warning"
  ...
  threshold = "70"
  alarm_actions = [
    aws_autoscaling_policy.content-other-add-instance.arn
  ]
  dimensions = {
    ClusterName = "production-other"
  }
}
