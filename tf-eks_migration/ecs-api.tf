resource "aws_ecs_cluster" "content-api" {
  name = "content-api"
}

resource "aws_launch_configuration" "content-api" {
  name          = "content-api"
...
resource "aws_autoscaling_group" "content-api" {
  name                      = "content-api"
  min_size                  = var.public.ecs-api.min_size
  max_size                  = var.public.ecs-api.max_size
  health_check_type         = "EC2"
  launch_configuration      = aws_launch_configuration.content-api.name
  vpc_zone_identifier       = [
  ...
  tags = [
    {
      key                 = "Name"
      value               = "ecs-content-api"
      propagate_at_launch = "1"
    }
  ]
}

resource "aws_autoscaling_policy" "content-api-add-instance" {
  name                   = "content-api-add-instance"
  scaling_adjustment     = 1
  adjustment_type        = "ChangeInCapacity"
  cooldown               = 300
  autoscaling_group_name = aws_autoscaling_group.content-api.name
}

resource "aws_cloudwatch_metric_alarm" "content-api-memory-warning" {
  alarm_name = "content-api-memory-warning"
  ...
  threshold = "70"
  alarm_actions = [
    aws_autoscaling_policy.content-api-add-instance.arn
  ]
  dimensions = {
    ClusterName = "content-api"
  }
}
