resource "aws_ecs_cluster" "content-coding-challenges" {
  name = "codereview"
}

resource "aws_launch_configuration" "content-coding-challenges" {
  name          = "content-coding-challenges"
...
resource "aws_autoscaling_group" "content-coding-challenges" {
  name                      = "content-coding-challenges"
  min_size                  = var.public.ecs-codereview.min_size
  max_size                  = var.public.ecs-codereview.max_size
  health_check_type         = "EC2"
  launch_configuration      = aws_launch_configuration.content-coding-challenges.name
  vpc_zone_identifier       = [
  ...
  tags = [
    {
      key                 = "Name"
      value               = "ecs-content-coding-challenge"
      propagate_at_launch = "1"
    }
  ]
}

resource "aws_autoscaling_policy" "content-coding-challenges-add-instance" {
  name                   = "content-coding-challenges-add-instance"
  scaling_adjustment     = 1
  adjustment_type        = "ChangeInCapacity"
  cooldown               = 300
  autoscaling_group_name = aws_autoscaling_group.content-coding-challenges.name
}

resource "aws_cloudwatch_metric_alarm" "content-coding-challenges-memory-warning" {
  alarm_name = "content-coding-challenges-memory-warning"
  ...
  threshold = "70"
  alarm_actions = [
    aws_autoscaling_policy.content-coding-challenges-add-instance.arn
  ]
  dimensions = {
    ClusterName = "codereview"
  }
}
