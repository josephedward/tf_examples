resource "aws_apigatewayv2_api" "main" {
  name                       = "emerson-api"
  description                = "Websocket on AWS Simple Testing API"
...
resource "aws_apigatewayv2_integration" "chuck-dotnetcore" {
  api_id             = aws_apigatewayv2_api.main.id
  integration_uri    = aws_lambda_function.chuck-dotnetcore.invoke_arn
  integration_type   = "AWS_PROXY"
  integration_method = "POST"
}
...
resource "aws_apigatewayv2_route" "test" {
  api_id    = aws_apigatewayv2_api.main.id
  route_key = "chuck-dotnetcore"
  target    = "integrations/${aws_apigatewayv2_integration.chuck-dotnetcore.id}"
}
